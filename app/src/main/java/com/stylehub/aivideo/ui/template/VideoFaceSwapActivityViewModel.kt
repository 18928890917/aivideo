 ...
 */
class VideoFaceSwapActivityUiData: BaseSwapActivityUiData() {
}

class VideoFaceSwapActivityViewModel(initialValue: VideoFaceSwapActivityUiData = VideoFaceSwapActivityUiData()) :
    BaseSwapActivityViewModel<VideoFaceSwapActivityUiData>(initialValue) {
    /**
     * 视频模板
     */
    ...
    override fun setIntentData(data: Template?) {
        ...
        if (data is FaceSwapVideoTemplateRespDataModel) {
            data.run {
                ...
                viewModelScope.launch {
                    ...
                    if (templateUrl != null) {
                        mutableData.largeVideoUrl = templateUrl
                    } else {
                        val bitmap = ImageUtil.getBitmap(uri = animate?.toUri())
                        templateComposedBitmap = bitmap
                        if (mutableData.largeImageBitmap == null) {
                            mutableData.largeImageBitmap = templateComposedBitmap
                        }
                    }
                    val headBitmap = ImageUtil.getBitmap(uri = templateFaceUrl?.toUri())
                    mutableData.bottomBarLeftHeadBitmap = headBitmap
                }
            }
        }
    }
    ...
    override fun onSwapClick() {
        ...
        //没有选择图片或模板
        if (videoFaceSwapTaskReqDataModel == null) {
     ...
     * 创建视频换脸任务
     */
    private suspend fun swapVideoFace() : VideoFaceSwapTaskRespDataModel? {

        val userId = LoginManager.getUserId() ?: return null
        val reqModel = videoFaceSwapTaskReqDataModel ?: return null
        ...
        return suspendCoroutine {
            ...
            api.createVideoFaceSwapTask(commonReq, userId)
                .enqueue(object : retrofit2.Callback<CommonRespModel<VideoFaceSwapTaskRespDataModel>> {
                    override fun onResponse(
                        call: retrofit2.Call<CommonRespModel<VideoFaceSwapTaskRespDataModel>>,
                        response: retrofit2.Response<CommonRespModel<VideoFaceSwapTaskRespDataModel>>
                    ) {
